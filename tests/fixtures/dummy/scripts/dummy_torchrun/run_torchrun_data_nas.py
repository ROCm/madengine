#!/usr/bin/env python3
"""
PyTorch Distributed Training with NAS Data Provider for madengine

This benchmark demonstrates distributed training with NAS data provider integration:
- Multi-node/multi-GPU distributed training with DDP
- NAS data provider support (mounted filesystem or downloaded data)
...
Usage:
  # K8s Multi-node with NAS data provider
  torchrun --nnodes=2 --nproc_per_node=2 --master_addr=... run_torchrun_data_nas.py
"""
...
# Data configuration - NAS may contain any files
# We validate that MAD_DATAHOME exists and has content (like run_data_nas.sh)
# No specific file required - synthetic data used for training benchmark
...
def print_header():
    ...
    print("=" * 70)
    print("madengine PyTorch Distributed Training with NAS Data Provider")
    print("=" * 70)
    ...
    if rank == 0:
        ...
        if not data_home_path.exists():
            ...
            try:
                ...
                if not dir_contents:
                    print(f"⚠️  Data home directory is EMPTY")
                    print(f"   This is okay for test environments")
                    print(f"   (data provider works but source is empty)")
                else:
                    print(f"✅ Data home has {len(dir_contents)} items")
                    # List first few files for verification
                    print(f"   Contents:")
                    for i, item in enumerate(dir_contents[:5]):
...
def main():
    ...
    if rank == 0:
        ...
        with open("training_results.txt", "w") as f:
            f.write(f"Training Results with NAS Data Provider\n")
            f.write(f"========================================\n")
