...
class TestPassthroughKeys:
    ...
    def test_launcher_alias_canonicalized(self):
        ...
        # --additional-context, so both paths accept identical spellings.
        cfg = make_cfg({"distributed": {"enabled": True, "launcher": "sglang_disagg"}})
        ctx, meta = ConfigTranslator.to_additional_context(cfg)
        assert ctx["distributed"]["launcher"] == "sglang-disagg"
...
class TestExtractedKeys:
    ...
    def test_false_keep_alive_omitted(self):
        cfg = make_cfg({})
        ctx, meta = ConfigTranslator.to_additional_context(cfg)
        assert "docker_keep_alive" not in ctx
        assert "docker_clean_cache" not in ctx

    def test_true_clean_cache_mapped(self):
        cfg = make_cfg(
            {
                "docker": {
                    "build_args": {},
        ...
        ctx, meta = ConfigTranslator.to_additional_context(cfg)
        assert ctx["docker_clean_cache"] is True
