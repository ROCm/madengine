...
class TestConflictDetection:
    ...
    def test_slurm_and_kubernetes_alias_conflict(self):
        cfg = make_cfg(
            {"slurm": {"partition": "gpu"}, "kubernetes": {"namespace": "default"}}
        )
        errors = ConfigValidator.validate(cfg)
        assert any("Cannot specify both" in e for e in errors)

    def test_require_pinned_image_known(self):
        cfg = make_cfg({"require_pinned_image": True})
        errors = ConfigValidator.validate(cfg)
