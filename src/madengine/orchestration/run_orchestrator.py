...
class RunOrchestrator:
    ...
    def _execute_local(self, manifest_file: str, timeout: int) -> Dict:
        ...
        if "context" in manifest:
            ...
            # Restore runtime-relevant context from manifest.
            # Keep runtime-detected values as priority, but bring missing keys
            # from manifest (especially docker_mounts for host path visibility).
            ...
            if "docker_env_vars" in manifest_context:
                if "docker_env_vars" not in self.context.ctx:
                    self.context.ctx["docker_env_vars"] = {}
                for key, value in manifest_context["docker_env_vars"].items():
                    if key not in self.context.ctx["docker_env_vars"]:
