...
class RunOrchestrator:
        ...
        try:
            ...
            try:
                if target == "bare_metal":
                    results = self._execute_bare_metal(manifest_file, timeout)
                elif target == "local" or target == "docker":
    ...
    def _execute_local(self, manifest_file: str, timeout: int) -> Dict:
        ...
        if "context" in manifest:
            ...
            if "docker_mounts" in manifest_context:
                ...
                for container_path, host_path in manifest_context[
                    "docker_mounts"
                ].items():
            ...
            if (
                "docker_gpus" in manifest_context
                and "docker_gpus" not in self.context.ctx
            ):
                self.context.ctx["docker_gpus"] = manifest_context["docker_gpus"]
            if (
                "gpu_vendor" in manifest_context
                and "gpu_vendor" not in self.context.ctx
            ):
    ...
    def _execute_bare_metal(self, manifest_file: str, timeout: int) -> Dict:
        """Execute models on bare metal (conda env, no Docker).

        Like ``_execute_local`` but skips Docker-image GPU filtering (images are
        synthetic bare-metal entries) and uses ``BareMetalRunner`` instead of
        ``ContainerRunner``. GPU detection still runs natively via the runtime
        context; scripts/common is still populated for pre/post scripts.
        """
        self.rich_console.print("[cyan]Executing on bare metal (conda)...[/cyan]\n")

        with open(manifest_file, "r") as f:
            manifest = json.load(f)

        # Initialize runtime context (native GPU detection, no Docker needed).
        self._init_runtime_context()
        self._show_node_info()
        ...
        # Restore host-level context from manifest (pre/post scripts, tools,
        # encapsulate, guest_os) so pre/post scripts and tools apply on bare metal.
        if "context" in manifest:
            manifest_context = manifest["context"]
            for key in (
                "tools",
                ...
                if key in manifest_context and key not in self.context.ctx:
                    self.context.ctx[key] = manifest_context[key]

        # Runtime --additional-context overrides manifest.
        if self.additional_context:
            for key in ("tools", "pre_scripts", "post_scripts", "encapsulate_script"):
                if key in self.additional_context:
                    self.context.ctx[key] = self.additional_context[key]

        # Populate scripts/common so pre/post scripts resolve.
        self._copy_scripts()
        ...
        self.rich_console.print("\n[green]✓ Bare-metal execution complete[/green]")
        self.rich_console.print(f"[dim]{'=' * 60}[/dim]\n")
        return results
    ...
    def _infer_deployment_target(self, config: Dict) -> str:
        ...
        - Presence of "bare_metal" field → bare-metal (conda, no Docker)
        - Presence of "k8s" or "kubernetes" field → k8s deployment
        - Presence of "slurm" field → slurm deployment
        - None present → local execution (Docker)

        Args:
            config: Configuration dictionary

        Returns:
            Deployment target: "bare_metal", "k8s", "slurm", or "local"
        """
        if "bare_metal" in config:
            return "bare_metal"
