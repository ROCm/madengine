...
echo "Stopping GPU power profiler..."
...
if [ ! -f "$PROFILER_START_FILE" ]; then
    echo "⚠️  Warning: Power profiler was not started - skipping"
    exit 0
fi

# Check if PID file exists
if [ ! -f "$PROFILER_PID_FILE" ]; then
    echo "⚠️  Warning: Power profiler PID file not found - profiler may not be running"
    exit 0
fi
...
echo "✓ Power profiler cleanup complete"

# Show profiler log if it exists
if [ -f "/tmp/gpu_info_power_profiler.log" ]; then
    echo "=== Power Profiler Log ==="
