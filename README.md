...
- **📂 ROCm Path** - Auto-detect **host** ROCm root; optional `docker_env_vars.MAD_ROCM_PATH`; in-container `ROCM_PATH` is resolved at Docker run (image OCI env + in-image probe, not host mirroring) — see [Configuration](docs/configuration.md#rocm-path-run-only)
...
**ROCm not in /opt/rocm:** Use top-level `MAD_ROCM_PATH` or `--rocm-path` for the **host**; for **in-container** paths, set `docker_env_vars.MAD_ROCM_PATH`, or let madengine resolve `ROCM_PATH` at run from the image and probe (see [Configuration](docs/configuration.md#rocm-path-run-only)).
