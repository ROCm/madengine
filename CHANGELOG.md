...
- **GPU detection no longer aborts every run on WSL2/DXG hosts** (ROCM-30980): On WSL2 the GPU is exposed through `/dev/dxg`, so there is no `/dev/kfd`, no KFD topology under `/sys/devices/virtual/kfd`, and no DRM render node. `amd-smi list -e --json` consequently reports `render`/`kfd_id`/`node_id`/`card` as the string `'N/A'`, and `Context.get_gpu_renderD_nodes()` cast that field with `int()` — raising `invalid literal for int() with base 10: 'N/A'`. Detection runs before any benchmark, so the run aborted with exit code 3 and *every* model was scored FAIL on a WSL runner, even though the GPU was fully functional (`hip_id` populated, HIP compute verified). An unparseable `render` value is now treated as "this host has no renderD node" and yields `None`, the value `get_gpu_arg()` already handles by omitting `--device=/dev/dri/renderD*`; `None` is returned rather than a partial list because callers index it by GPU id. Fixing the parse alone was not sufficient — `get_gpu_arg()` also passed `--device=/dev/kfd` unconditionally for AMD, which fails on WSL with "no such file or directory" — so new `Context.get_is_wsl()` detects the platform from the devices that are actually present (`/dev/dxg` without `/dev/kfd`, rather than a `/proc/version` kernel string) and exposes it as the `is_wsl` context key, overridable via `--additional-context`. When set, `ContainerRunner.get_wsl_gpu_arg()` emits `--device=/dev/dxg`, bind-mounts `libdxcore.so`, `librocdxg.so` and `dids.conf` into the container's default lookup paths (skipping any that are absent on the host, since Docker would otherwise create a directory that shadows the image's own copy), and sets `HSA_ENABLE_DXG_DETECTION=1`, which ROCm runtimes older than 7.13 require to probe `/dev/dxg`. Native Linux/KFD hosts are unaffected.
